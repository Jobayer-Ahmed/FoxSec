...
xhttp.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) {
        ...
        var idPass = response.idPass;
        for (let i = 0; i < idPass.length; i++) { // notice the usage of let instead of var
        		var d = $("<a class=\"" + idPass[i].siteName + "\"><i class=\"fa fa-" + idPass[i].siteName + "\"><span class=\"" + idPass[i].siteName + "\"></span></i></a>")
            .click(function(e){
            	colorChange();
              	var a = $("<span class=\"tle " + idPass[i].siteName + "\">" + idPass[i].siteName + "</span>");
            	var b = $("<div class=\"name " + idPass[i].siteName + "\">" + "<strong>USERNAME: </strong>" + idPass[i].usrName + "</div>");
            	var encrypted = CryptoJS.AES.encrypt(idPass[i].pass, "Secret Passphrase");
				var decrypted = CryptoJS.AES.decrypt(encrypted, "Secret Passphrase");
				var normal = decrypted.toString(CryptoJS.enc.Utf8);
				var xyz = encrypted;
            	var c = $("<div class=\"pass " + idPass[i].siteName + "\">" + "<strong>PASSWORD: </strong>" + xyz + "</div>").click(function() {
            		alert(normal)
            	});
            	$("#siteName").html(a);
            	$(".userName").html(b);
...
xhttp.open("GET", "https://jobayer-ahmed.github.io/FoxSec/assets/json/userName.json", true);
xhttp.send();
...
function colorChange() {
	...
	$('body').css("backgroundColor", colors[x]);
	$('.ico a').css("color", colors[x]);
    $('.goIcon').css("color", colors[x]);
